...
export function initDb() {
  ...
  const existingAdmin = db.prepare("SELECT * FROM users WHERE username = 'Admin' OR username = 'admin'").get() as any;

  if (existingAdmin) {
    db.prepare('UPDATE users SET username = ?, password_hash = ? WHERE id = ?').run('Admin', hashedPassword, existingAdmin.id);
    console.log('Admin credentials updated: Admin / ronaldocr7');
  } else {
    db.prepare('INSERT INTO users (username, password_hash) VALUES (?, ?)').run('Admin', hashedPassword);
    console.log('Admin user created: Admin / ronaldocr7');
  }
  ...
  if (categoryCount.count === 0) {
    ...
    // Get IDs
    const startersId = db.prepare("SELECT id FROM categories WHERE name = 'Starters'").get() as { id: number };
    const mainsId = db.prepare("SELECT id FROM categories WHERE name = 'Main Courses'").get() as { id: number };
    const seafoodId = db.prepare("SELECT id FROM categories WHERE name = 'Seafood'").get() as { id: number };
    const vegetarianId = db.prepare("SELECT id FROM categories WHERE name = 'Vegetarian'").get() as { id: number };
    const dessertsId = db.prepare("SELECT id FROM categories WHERE name = 'Desserts'").get() as { id: number };
    const beveragesId = db.prepare("SELECT id FROM categories WHERE name = 'Beverages'").get() as { id: number };
  ...
  if (postsCount.count === 0) {
    const insertPost = db.prepare('INSERT INTO posts (title, content, image_url, type, date) VALUES (?, ?, ?, ?, ?)');
    insertPost.run(
      'Grand Opening Night',
